...
-- Trong bảng production.products lấy ra 3 thuộc tính product_id, product_name, list_price, thuộc tính thứ 4 là lower_1_level_price sẽ lấy bằng cách sắp xếp tập dữ liệu theo list_price tăng dần và lấy cột list_price lag ngược về 1 GT. VD: list price tăng dần chỉ là 3,4,6 thì lower_1_level_price sẽ là NULL,3,4. VD row 3 mang giá trị row 2 ở trường đó

-- LAG ( expression [, offset [, default] ] ) OVER ( [ query_partition_clause ] order_by_clause ) sửa đổi 1 trường định lấy thành cái gì -> lấy trường expression lag về 1 lượng offset(default là 1), nếu ngoài phạm vi sẽ lấy giá trị default(mặc định là NULL), sẽ PARTITION BY nếu có và ORDER BY cái gì bên ngoài
...
FROM production.products;
-- Lấy 4 trường ban đầu từ production.products, sau đó lấy trường cuối lower_2_level_price bằng cách chia nó thành các nhóm theo model_year, trong mỗi nhóm sắp xếp theo list_price giảm dần(nó cũng khiến data trên bảng hiển thị ra sẽ tự chia nhóm theo model_year và hiển thị list_price giảm dần) và lấy lag lại 2 đơn vị trong từng nhóm tức đang sắp xếp giảm dần thì tại 1 vị trí nào đó sẽ lấy lùi lại 2 item là lớn hơn giá trị 2 hiện tại. VD: 5,4,3,2 sẽ lấy NULL,NULL,5,4 
